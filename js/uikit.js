...
(function (global, factory) {
      ...
      .concat(
        ancestors.filter(
          (parent) =>
            ...
            (!scrollable ||
              parent.scrollHeight > offsetViewport(parent).height),
        ),
      )
      .reverse();
  }
          ...
          columns.forEach((column, i) =>
            column.forEach((el, j) =>
              css(
                ...
                !scrolled && !translates
                  ? ""
                  : `translateY(${(translates && -translates[i][j]) + (scrolled ? (i % 2 ? scrolled : scrolled / 8) : 0)}px)`,
              ),
            ),
          );
        },
  ...
  var sticky = {
    ...
    observe: [
      resize({ target: ({ $el }) => [$el, document.documentElement] }),
      scroll$1(),
    ],
    events: [
      {
    ...
    computed: {
      ...
      start({ start }) {
        ...
        return toPx(
            ...
            ((viewport = (1 - viewport) * 100) && `${viewport}vh+${viewport}%`),
          "height",
          this.target,
          true,
        );
